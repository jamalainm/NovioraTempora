...
def roll_init(character):
    ...
    """
    bonus = calc_bonus(character.db.ingenia['pernīcitās'])
...
def get_defense(attacker, defender):
    ...
    bonus = calc_bonus(defender.db.ingenia['pernīcitās'])
...
#                attacker.location.msg_contents(f"|gWEAPON|n")
    bonus = calc_bonus(attacker.db.ingenia['vīrēs'])
    if weapon:
        damage_value = randint(1,weapon.db.vīs) + bonus
    else:
...
def apply_damage(defender, damage):
    ...
    defender.db.pv['nunc'] -= damage  # Reduce defender's HP by the damage dealt.
    # If this reduces it to 0 or less, set HP to 0.
    if defender.db.pv['nunc'] <= 0:
        defender.db.pv['nunc'] = 0
...
def at_defeat(defeated):
    ...
    defeated.location.msg_contents(f"|c%s|n |rest vict{us_a_um('nom_sg',caller.db.sexus)}!|n" % defeated,exclude=defeated)
    defeated.msg(f"|rTū es vict{us_a_um('nom_sg',caller.db.sexus)}!|n")
...
def resolve_attack(attacker, defender, attack_value=None, defense_value=None):
    ...
    for thing in stuff:
        if thing.db.tenētur:
            if thing.db.tenētur == good_hand:
                weapon = thing.db.formae['abl_sg'][0]
    if not weapon:
        weapon = 'manū'
    ...
    if (attack_value < defense_value or attack_roll == 1) and attack_roll != 20:
        attacker.location.msg_contents("|c%s|n |C%s|n |w%s|n |gnōn percussit|n!" % (attacker.db.formae['nom_sg'][0], defender.db.formae['acc_sg'][0],weapon),exclude=(attacker,defender))
        attacker.msg("|cTū|n |C%s|n |w%s|n |rnōn percussistī|n!" % (defender.db.formae['acc_sg'][0],weapon))
        defender.msg("|C%s|n |ctē|n |w%s|n |gnōn percussit|n!" % (attacker,weapon))
    else:
        if attack_roll == 20:
            damage_value = 2 * (get_damage(attacker, defender))  # Calculate damage value.
            attacker.location.msg_contents("|c%s|n |C%s|n |w%s|n |rFORTISSIME percussit|n!" % (attacker.db.formae['nom_sg'][0], defender.db.formae['acc_sg'][0],weapon),exclude=(attacker,defender))
            attacker.msg("|cTū|n |C%s|n |w%s|n |gFORTISSIME percussistī|n!" % (defender.db.formae['acc_sg'][0],weapon))
            defender.msg("|C%s|n |ctē|n |w%s|n |rFORTISSIME percussit|n!" % (attacker,weapon))
        else:
            damage_value = get_damage(attacker, defender)  # Calculate damage value.
        # Announce damage dealt and apply damage.
            attacker.location.msg_contents("|c%s|n |C%s|n |w%s|n |rpercussit|n!" % (attacker.db.formae['nom_sg'][0], defender.db.formae['acc_sg'][0],weapon),exclude=(attacker,defender))
            attacker.msg("|cTū|n |C%s|n |w%s|n |gpercussistī|n!" % (defender.db.formae['acc_sg'][0],weapon))
            defender.msg("|C%s|n |ctē|n |w%s|n |rpercussit|n!" % (attacker,weapon))
...
#
    def at_before_move(self, destination):
        ...
        if is_in_combat(self):
            self.msg("Tibi pugnantī discēdere nōn licet!")
        ...
        if self.db.pv['nunc'] <= 0:
            self.msg(f"Tū discēdere nōn potes: es vict{'a' if self.db.gender == 1 else 'us' if self.db.gender == 2 else 'um'} atque fortasse moreris!")
...
#        character.msg("|wNunc tibi agere licet! Vita: |c%i|n|w.|n" % character.db.hp['current'])
        character.msg("\n|wVīta: %i/%i)|n " % (character.db.pv['nunc'],character.db.pv['max']))
    ...
    def next_turn(self):
        ...
        if disengage_check:  # All characters have disengaged
            self.obj.msg_contents("|wOmnēs cessērunt! Pugna peracta est!|n")
...
#                    hostile.location.msg_contents(f"{hostile} rolled a {hostile_dex_check}!")
                    if hostile_dex_check > coward_dex_check:
                        flee_check = False
                        coward.location.msg_contents('|rFugere nōn potes!|n')
                        break
                if flee_check:
                    coward.location.msg_contents(f"|r{coward} fūgit|n!",exclude=coward)
                    coward.msg("|gFūgistī|n!")
                    exits = [
...
#                fighter.db.fighting = False
        ...
        if defeated_characters == (
            ...
            self.obj.msg_contents(f"|c%s|n |wvīcit! Pugna est peracta!|n" % LastStanding,exclude=LastStanding)
            LastStanding.msg(f"|gTū vīcistī!! Pugna est peracta!|n")
            self.stop()  # Stop this script and end combat.
...
class CmdFight(MuxCommand):
    ...
    def func(self):
        ...
        attacker = caller
        defender = target
#        defender = caller.search(self.args)
#        attacker.msg(f"{defender} is your target")

        if not defender:  # No valid target given.
            caller.msg("Quem pugnāre velis?")
            return
        if not defender.db.pv:
            caller.msg(f"Tibi {defender.db.form['acc_sg'][0]} pugnāre nōn licet!")
            return

        # Adjusting so it recognizes my hp syntax
        if not defender.db.pv['nunc']:  # Target object has no HP left or to begin with
            caller.msg(f"{defender.db.formae['nom_sg'][0]} iam vict{us_a_um('nom_sg',defender.db.sexus)} est!")
            return

        if attacker == defender:  # Target and attacker are the same
            caller.msg("Tū tē pugnāre nōn potes!!")
            return

        # Adjusting so script recognizes my syntax for hp
        if not caller.db.pv['nunc']:  # If you don't have any hp
            caller.msg(f"Vict{us_a_um('nom_sg',caller.db.sexus)} tū pugnāre nōn potes!")
            return
        if is_in_combat(caller):  # Already in a fight
            caller.msg("Tū iam pugnās!")
            return
...
#                        fighters.append(thing)  # ...then add it to the fight.
        ...
        if len(fighters) <= 1:  # If you're the only able fighter in the room
            caller.msg("Nēmō est quōcum pugnāre potes!")
            return
        if here.db.combat_turnhandler:  # If there's already a fight going on...
            here.msg_contents("|c%s|n pugnam commīsit!" % caller, exclude=caller)
            caller.msg("Tū pugnam commīsistī!")
            here.db.combat_turnhandler.join_fight(caller)  # Join the fight!
            return
        here.msg_contents("|c%s|n |C%s|n pugnāre coepit!" % (caller,defender.db.formae['acc_sg'][0]),exclude=(caller,defender))
        caller.msg(f"Tū |c%s|n pugnāre coepistī!" % defender.db.formae['acc_sg'][0])
        defender.msg(f"|c{caller}|n tē pugnāre coepit!")
...
class CmdAttack(MuxCommand):
    ...
    def func(self):
        ...
        if not defender:  # No valid target given.
            caller.msg("Quem peter velis?")
            return

        # Adjusting so it recognizes my hp syntax
        if not defender.db.pv:
            caller.msg(f"Tibi {defender.db.formae['acc_sg'][0]} pugnāre nōn licet!")
            return
        if not defender.db.pv['nunc']:  # Target object has no HP left or to begin with
            caller.msg(f"{defender.db.nom_sg[0]} iam vict{us_a_um('nom_sg',defender.db.sexus)} est!")           
            return
        ...
        # Adjusting so it recognizes my hp syntax
        if not caller.db.pv['nunc']:  # Can't attack if you have no HP.
            caller.msg(f"Vict{us_a_um('nom_sg',caller.db.sexus)} tū pugnāre nōn potes!")
            return

        # after I moved the top block, the above was just after 'is_turn'

        if not is_in_combat(caller):  # If not in combat, can't attack.
            # Join fight if defender is already fighting
            if is_in_combat(defender):  # Already in a fight
                here.msg_contents("|c%s|n pugnam commīsit!" % caller, exclude=(caller,defender))
                caller.msg("Tū pugnam commīsistī!")
                defender.msg(f"|c%s|n tēcum pugnam commīsit!" % caller)
            ...
            # start fight if defender not already fighting
                ...
                if len(fighters) <= 1:  # If you're the only able fighter in the room
                    ...
                    return
                if here.db.combat_turnhandler:  # If there's already a fight going on...
                    here.msg_contents("|c%s|n pugnam cum |C%s|n commīsit!" % (caller,defender.db.formae['abl_sg'][0]), exclude=(caller,defender))
                    caller.msg("|cTū|n cum |C%s|n pugnam comīsistī!" % defender.db.abl_sg[0])
                    defender.msg("|C%s|n |ctē|ncum pugnam comīsit!" % caller)
                    here.db.combat_turnhandler.join_fight(caller)  # Join the fight!
                    return
                here.msg_contents("|c%s|n pugnam cum |C%s|n commīsit!" % (caller,defender.db.formae['abl_sg'][0]), exclude=(caller,defender))
                caller.msg("|cTū|n cum |C%s|n pugnam comīsistī!" % defender.db.formae['abl_sg'][0])
                defender.msg("|C%s|n |ctē|ncum pugnam comīsit!" % caller)
                # Add a turn handler script to the room, which starts combat.
                # change to this script
                here.scripts.add("world.tb_basic.TBBasicTurnHandler")
...
#            caller.msg("You can only do that in combat. (see: help fight)")
#            return

        # Adding to deal with attacking someone not in combat
        if not is_in_combat(defender):
            here.msg_contents("|c%s|n pugnam cum |C%s|n commisit!" % (caller,defender.db.formae['abl_sg'][0]), exclude=(caller,defender))
            caller.msg("|cTū|n cum |C%s|n pugnam comīsistī!" % defender.db.formae['abl_sg'][0])
            defender.msg("|C%s|n |ctē|ncum pugnam comīsit!" % caller)
            here.db.combat_turnhandler.join_fight(defender)  # Join the fight!
            return

        if not is_turn(caller):  # If it's not your turn, can't attack.
...
#            caller.msg("You can only do that on your turn.")
        ...
        resolve_attack(attacker, defender)
        spend_action(caller, 1, action_name="attack")  # Use up one action.
...
class CmdPass(MuxCommand):
    ...
    def func(self):
        ...
        if not is_in_combat(caller):  # Can only pass a turn in combat.
            caller.msg("Nisi pugnāns, hoc agere nōn potes. (vidē: pugnā)")
            return

        if not is_turn(caller):  # Can only pass if it's your turn.
#            caller.msg("You can only do that on your turn.")
            return

        caller.location.msg_contents("|C%s|n nihil ēgit." % caller, exclude=caller)
        caller.msg('|cTū|n nihil ēgistī.')
        spend_action(caller, "all", action_name="pass")  # Spend all remaining actions.
...
class CmdDisengage(MuxCommand):
    ...
    def func(self):
        ...
        if not is_in_combat(caller):  # If you're not in combat
            caller.msg("Nisi pugnāns, hoc nōn agere potes. (vidē: pugna)")
            return

        if not is_turn(caller):  # If it's not your turn
            caller.msg("Hōc tempore tū agere nōn potes.")
            return

        caller.location.msg_contents("|C%s|n |wcessit: pugnāre nōn vult|n." % caller, exclude=caller)
        caller.msg("|gCessistī|n.")
        spend_action(caller, "all", action_name="disengage")  # Spend all remaining actions.
        """
...
class CmdRest(MuxCommand):
    ...
    def func(self):
        ...
        if is_in_combat(caller):  # If you're in combat
            caller.msg("Tibi pugnantī requiescere nōn licet.")
            return

        # Adjusting so it recognizes the syntax of my hp
        caller.db.pv['nunc'] = caller.db.pv['max']  # Set current HP to maximum
        caller.location.msg_contents("|c%s|n requiēvit ut vitam reficeret." % caller,exclude=caller)
        caller.msg("Requiēvistī ut vitam reficerēs.")
        """
...
class CmdFlee(MuxCommand):
    ...
    def func(self):
        ...
        if not is_in_combat(caller):  # If you're not in combat
            caller.msg("Nisi pugnāns, hoc agere nōn potes. (vidē: pugna)")
            return
...
#            caller.msg("You can only do that on your turn.")
        ...
        caller.location.msg_contents("|C%s|n fugere conātur!" % caller,exclude=caller)
        caller.msg("|cFugere conāris|n!")
        spend_action(caller, "all", action_name="flee")  # Spend all remaining actions.
        """
        The action_name kwarg sets the character's last action to "disengage", which is checked by
        the turn handler script to see if all fighters have disengaged.
        """
